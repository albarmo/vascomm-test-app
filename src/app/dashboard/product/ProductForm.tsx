...
import { createProduct } from '@/app/helpers/product_server';
...
const ProductCreationForm = () => {
    ...
    const { mutation } =
        useCustomMutation(createProduct);

    // const [selectedFiles, setSelectedFiles] = useState<File | null>(null);
    // const handleUploadDocument = (event: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    const onSubmit: SubmitHandler<Inputs> = async (data) => {
        try {
            const resp = await mutation.mutateAsync({ title: data?.title, price: data?.price, status: data.status, image: file });
            if (resp.status === 200) {
        ...
        <section className='w-full bg-white'>
            <div className='text-left leading-6 font-sans p-5'>
                <form onSubmit={handleSubmit(onSubmit)} className='grid grid-cols-1 gap-y-2 py-5 text-gray-600'>
                    ...
                    <input id='title' {...register("title")} type='text' placeholder='Isi nama' className='border border-gray-500 rounded-sm h-10 w-full p-2' />
                    <p className='text-red-400'>{errors.title?.message}</p>

                    <label htmlFor='price'>Harga</label>
                    <input id='price' {...register("price")} type='number' placeholder='0' className='border border-gray-500 rounded-sm h-10 w-full p-2' />
                    <p className='text-red-400'>{errors.price?.message}</p>


                    <label htmlFor='status'>Status</label>
                    <input id='status' {...register("status")} type='checkbox' defaultChecked={true} className='border border-gray-500 rounded-sm h-5 w-5 p-2' />
